...
public class GPUImageToneCurveFilter extends GPUImageFilter {
            ...
            public void run() {
                ...
                if (GPUImageToneCurveFilter.this.mRedCurve.size() >= 256 && GPUImageToneCurveFilter.this.mGreenCurve.size() >= 256 && GPUImageToneCurveFilter.this.mBlueCurve.size() >= 256 && GPUImageToneCurveFilter.this.mRgbCompositeCurve.size() >= 256) {
                    ...
                    for (int currentCurveIndex = 0; currentCurveIndex < 256; currentCurveIndex++) {
                        toneCurveByteArray[(currentCurveIndex * 4) + 2] = (byte) (((int) Math.min(Math.max(((float) currentCurveIndex) + ((Float) GPUImageToneCurveFilter.this.mBlueCurve.get(currentCurveIndex)).floatValue() + ((Float) GPUImageToneCurveFilter.this.mRgbCompositeCurve.get(currentCurveIndex)).floatValue(), 0.0f), 255.0f)) & 255);
                        toneCurveByteArray[(currentCurveIndex * 4) + 1] = (byte) (((int) Math.min(Math.max(((float) currentCurveIndex) + ((Float) GPUImageToneCurveFilter.this.mGreenCurve.get(currentCurveIndex)).floatValue() + ((Float) GPUImageToneCurveFilter.this.mRgbCompositeCurve.get(currentCurveIndex)).floatValue(), 0.0f), 255.0f)) & 255);
                        toneCurveByteArray[currentCurveIndex * 4] = (byte) (((int) Math.min(Math.max(((float) currentCurveIndex) + ((Float) GPUImageToneCurveFilter.this.mRedCurve.get(currentCurveIndex)).floatValue() + ((Float) GPUImageToneCurveFilter.this.mRgbCompositeCurve.get(currentCurveIndex)).floatValue(), 0.0f), 255.0f)) & 255);
    ...
    private ArrayList<Point> createSplineCurve2(Point[] points) {
        ...
        for (int i2 = 0; i2 < n - 1; i2++) {
            Point cur = pointArr[i2];
            Point next = pointArr[i2 + 1];
            int x = cur.x;
            while (x < next.x) {
                double t = ((double) (x - cur.x)) / ((double) (next.x - cur.x));
                double a = 1.0d - t;
                double b = t;
                ArrayList<Double> sdA2 = sdA;
                int n2 = n;
        ...
        int i4 = n;
        if (output.size() == 255) {
            output.add(pointArr[pointArr.length - 1]);
        }
        return output;
    }
    ...
    private ArrayList<Double> createSecondDerivative(Point[] points) {
        ...
        int i = 1;
        while (i < n - 1) {
            Point P1 = pointArr[i - 1];
            Point P2 = pointArr[i];
            Point P3 = pointArr[i + 1];
            matrix[i][c3] = ((double) (P2.x - P1.x)) / 6.0d;
            matrix[i][c] = ((double) (P3.x - P1.x)) / 3.0d;
            matrix[i][c2] = ((double) (P3.x - P2.x)) / 6.0d;
